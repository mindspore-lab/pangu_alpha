...
def load_model(model_config, is_eval):
    ...
    print("data_parallel_num : ", data_parallel_num, "model_parallel_num : ", model_parallel_num)
    ...
    fake_input = ms.Tensor(np.ones(shape=(1, config.seq_length + 1)), mstype.int32)
    input_mask = ms.Tensor(np.ones(shape=(1, config.seq_length)), ms.float32)

    if model_config.distribute == "True":
        predict_layout = model.infer_predict_layout(fake_input, input_mask)
    else:
        predict_layout = None
