...
from src.model import VirtualDatasetOneInputCell, CrossEntropyLoss, PanguAlphaWithLoss2, PanguAlpha, PanguAlphaConfig
...
def load_model(model_config, is_eval):
    ...
    fake_input = ms.Tensor(np.ones(shape=(config.batch_size, config.seq_length + 1)), ms.int32)
    mask_ids = ms.Tensor(np.ones(shape=(config.batch_size, config.seq_length)), ms.float32)
