...
class Mapping_output(nn.Cell):
    ...
    def __init__(self, config, input_size, output_size, scale=1.0):
        ...
        self.dtype = config.compute_dtype
        self.cast = P.Cast()
        self.add = P.Add().shard(((dp, mp), (mp,)))
        self.matmul = P.MatMul().shard(((dp, 1), (1, mp)))
...
class output(nn.Cell):
    ...
    def __init__(self, config, scale=1.0):
        ...
        output_size = config.hidden_size * config.expert_num
        dp = config.parallel_config.data_parallel
        mp = config.parallel_config.model_parallel
        self.mapping = Mapping_output(config, input_size, output_size)
        self.projection = Mapping(config, output_size, input_size, scale)
        self.activation = nn.GELU()
        self.activation.gelu.shard(((dp, 1, mp),))
        self.dropout = _Dropout(1 - config.dropout_rate)
        self.dropout.dropout_gen_mask.shard(((dp, 1, 1),))
        self.dropout.dropout_do_mask.shard(((dp, 1, 1),))
...
class EmbeddingLookup(nn.Cell):
    ...
    def __init__(self, config):
        ...
        if config.word_embedding_name:
            embedding_path = os.path.join(config.load_ckpt_path, config.word_embedding_name)
        ...
        if config.word_emb_dp:
            ...
            self.gather = P.Gather().shard(((mp, 1), (1, 1)))
