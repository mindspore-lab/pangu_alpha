...
from src.model import PanguAlphaConfig, PanguAlpha, EvalNet_p
from src.utils.model_utils import get_ckpt_file_list, eval_load_file, infer_load_file
...
def load_model(model_config, is_eval):
    ...
    if model_config.distribute == "True":
        ...
        print('device num is : ', device_num)
        ...
        device_num = 1
        model_config.model_parallel_num = 1

    data_parallel_num = int(device_num / model_config.model_parallel_num)
    batch_size = model_config.per_batch_size * data_parallel_num
    model_parallel_num = model_config.model_parallel_num
    ...
    pangu_model = ms.Model(pangu)
    fake_input = Tensor(np.ones(shape=(1, config.seq_length)), mstype.int32)

    if model_config.distribute == "True":
        predict_layout = pangu_model.infer_predict_layout(fake_input)
    else:
        predict_layout = None

    ckpt_file_list = get_ckpt_file_list(os.path.join(model_config.ckpt_path, model_config.ckpt_dir),
                                        ckpt_name_prefix=model_config.ckpt_name_prefix, ckpt_num=model_config.ckpt_num)
